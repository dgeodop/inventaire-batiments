...
exports.batOfEtabl = function(req, res) {
	...
	var query = 'SELECT id_bat_dgeo, bat.id_bat, bat.nom_bat, lat, lng FROM bat, bat_dgeo WHERE bat.id_bat=bat_dgeo.id_bat AND util=1 AND id_etabl=$1 AND lng IS NOT NULL ORDER BY nom_bat';
	pg.connect(connectString, function(err, client, done) {
		...
		client.query(query, [idEtabl], function(err, result) {
		  ...
		  if(err) { return console.error('ctrlLogin.liste', err); }
			...
			res.render('carte', {data:data, idEtabl: idEtabl});
		});
	});
}
